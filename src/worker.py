...
from jobs import BASE_DIR, clear_active_job_id, guess_media_ext, has_media, job_folder, load_job, now_iso, save_job
...
async def _upload_review_copy(bot, review_chat_id: int, final_path: Path, job_id: str, make_text: str) -> tuple[int, str]:
    ...
    try:
        with final_path.open("rb") as fh:
            message = await bot.send_video(chat_id=review_chat_id, video=fh, caption=caption, supports_streaming=True)
        return message.id, "video"
    except Exception:
        with final_path.open("rb") as fh:
            message = await bot.send_document(chat_id=review_chat_id, document=fh, caption=caption)
...
async def process_job(cfg, job_id: str, bot=None, notify_chat_id: int | None = None) -> None:
    ...
    try:
        ...
        if bot and review_chat_id is not None:
            ...
            save_job(template_name, job)
            try:
                review_message_id, upload_kind = await _upload_review_copy(bot, int(review_chat_id), final_path, job_id, make_text)
                ...
                save_job(template_name, job)
                # The review copy is now safely stored in Telegram; keep the manifest,
                # but remove the large local final video so completed jobs don't grow disk usage.
        ...
        if bot and notify_chat_id is not None:
            await bot.send_message(chat_id=notify_chat_id, text=f"Job {job_id} finished. Review copy uploaded.")
    except Exception as exc:
        ...
        clear_active_job_id(template_name)
        if bot and notify_chat_id is not None:
            await bot.send_message(chat_id=notify_chat_id, text=f"Job {job_id} failed: {type(exc).__name__}: {exc}")
