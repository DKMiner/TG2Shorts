...
async def _do_publish(job: dict, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id=int(job["command_chat_id"]),
            text=f"Publish failed: {type(exc).__name__}: {exc}",
        )
    finally:
...
async def publish_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action in {"edit_title", "edit_description"}:
        ...
        save_runtime_job(job)
        field = "title" if job["edit_mode"] == "title" else "description"
...
async def publish_edit_reply(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    runtime_pointer = BASE_DIR / "data" / "jobs" / "runtime" / "active.json"
    if not runtime_pointer.exists():
        return
    try:
        import json
        job_id = str(json.loads(runtime_pointer.read_text(encoding="utf-8"))["job_id"])
    except Exception:
        return
    ...
    if not job or job.get("kind") != "youtube_publish":
        return
    if int(job.get("command_chat_id", 0)) != int(update.effective_chat.id):
        return
    if not job.get("edit_mode"):
        return

    if job.get("edit_prompt_message_id") and (
        msg.reply_to_message is None or msg.reply_to_message.message_id != job["edit_prompt_message_id"]
    ):
        return
